...
function HelpPage({ profile }) {
  ...
  const TAB = isAdmin ? 'admin' : isHQ ? 'hq' : 'manager';
  const [tab, setTab] = useState(TAB);

  const Section = ({ icon, title, children }) => (
    <div style={{ marginBottom:24 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, paddingBottom:8, borderBottom:'2px solid var(--sidebar)' }}>
        <span style={{ fontSize:18 }}>{icon}</span>
        <span style={{ fontSize:14, fontWeight:700, color:'var(--text)' }}>{title}</span>
      </div>
      <div style={{ fontSize:13, color:'var(--text)', lineHeight:1.8 }}>{children}</div>
    </div>
  );

  const Step = ({ n, text }) => (
    <div style={{ display:'flex', gap:10, marginBottom:8, alignItems:'flex-start' }}>
      <span style={{ background:'var(--sidebar)', color:'var(--sidebar-text)', borderRadius:'50%', width:22, height:22, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, flexShrink:0, marginTop:1 }}>{n}</span>
      <span style={{ fontSize:13, color:'var(--text)', lineHeight:1.7 }}>{text}</span>
    </div>
  );

  const Tip = ({ text }) => (
    <div style={{ background:'#fff8e1', border:'1px solid #ffcc80', borderRadius:'var(--radius)', padding:'8px 12px', fontSize:12, color:'#6d4c41', marginTop:8, marginBottom:8 }}>
      💡 {text}
    </div>
  );

  const Badge = ({ label, color='var(--accent)' }) => (
    <span style={{ display:'inline-flex', padding:'2px 8px', background:'#fff3e0', border:`1px solid ${color}`, borderRadius:4, fontSize:11, fontWeight:600, color, marginRight:4 }}>{label}</span>
  );
    ...
    <div>
      ...
      {tab === 'admin' && (
        <div className="card" style={{ padding:'24px 28px' }}>
          <div style={{ marginBottom:20 }}>
            <Badge label="ADMIN" color="#E65100"/>
            <span style={{ fontSize:13, color:'var(--text2)', marginLeft:8 }}>관리자는 모든 메뉴에 접근할 수 있으며, 시스템 전체를 관리합니다.</span>
          </div>

          <Section icon="👥" title="사용자 관리">
            <Step n={1} text="사이드바 [관리자] → 사용자 관리 클릭"/>
            <Step n={2} text="신규 가입 회원이 '대기 중'으로 표시되면 ✓ 승인 버튼을 눌러 승인합니다."/>
            <Step n={3} text="'관리자로' 버튼으로 특정 사용자에게 관리자 권한을 부여할 수 있습니다."/>
            <Tip text="매니저가 앱에 가입하면 반드시 관리자가 승인해야 로그인이 가능합니다."/>
          </Section>

          <Section icon="📢" title="공지사항">
            <Step n={1} text="사이드바 [관리자] → 공지사항 클릭"/>
            <Step n={2} text="'+ 공지사항 작성' 버튼 클릭 후 제목과 내용을 입력합니다."/>
            <Step n={3} text="'등록' 버튼을 눌러 저장하면 담당자·매니저 모두 열람 가능합니다."/>
            <Step n={4} text="공지 항목 클릭 시 내용이 펼쳐지며, 삭제 버튼으로 삭제할 수 있습니다."/>
          </Section>

          <Section icon="🛍️" title="브랜드/상품 관리">
            <Step n={1} text="사이드바 [본사] → 상품관리 클릭 시 전체 상품 현황 조회"/>
            <Step n={2} text="[상품추가] 서브메뉴: 브랜드 등록 후 상품명·판매가를 직접 입력하거나 엑셀 파일로 일괄 업로드"/>
            <Tip text="파일 업로드 양식: 브랜드명 / 상품명 / 판매가 — 헤더 행 필수"/>
          </Section>

          <Section icon="📦" title="재고관리">
            <Step n={1} text="[센터재고]: 창고 보유 재고를 브랜드·상품코드·수량으로 등록 및 관리"/>
            <Step n={2} text="[매장재고]: 점포/지점별 현재 재고 수량 확인 및 인라인 수정"/>
            <Step n={3} text="[안전재고]: 최근 1달 판매수량 기준으로 부족 재고 파악, 발주요청 가능"/>
            <Tip text="매니저가 판매를 입력하면 매장재고에서 자동 차감되지 않습니다. 재고는 직접 관리해야 합니다."/>
          </Section>

          <Section icon="👔" title="매니저관리 & QR">
            <Step n={1} text="사이드바 [본사] → 매니저관리 클릭 시 전체 매니저 현황 조회"/>
            <Step n={2} text="각 매니저 행의 '수정' 버튼으로 이름·점포·지점 정보 수정 가능"/>
            <Step n={3} text="'📱 QR' 버튼 클릭 시 해당 매니저의 회원가입 QR코드 팝업"/>
            <Step n={4} text="QR 팝업에서 URL 복사 또는 인쇄 가능 — 매장 카운터에 비치 권장"/>
            <Tip text="QR코드 URL 형식: 앱주소?m=매니저ID — 고객이 스캔하면 해당 매니저와 자동 연결"/>
          </Section>
        </div>
      )}
      ...
      {tab === 'hq' && (
        <div className="card" style={{ padding:'24px 28px' }}>
          <div style={{ marginBottom:20 }}>
            <Badge label="담당자" />
            <span style={{ fontSize:13, color:'var(--text2)', marginLeft:8 }}>담당자는 본사 전체 데이터를 조회·관리할 수 있습니다. (사용자 관리 제외)</span>
          </div>

          <Section icon="🏠" title="홈 대시보드">
            <Step n={1} text="로그인하면 자동으로 홈 대시보드가 열립니다."/>
            <Step n={2} text="당월 1일 ~ 어제까지의 누적 판매매출 합계를 확인할 수 있습니다."/>
            <Step n={3} text="하단 테이블에서 점포/지점별 매출금액과 비중(%)을 한눈에 파악합니다."/>
          </Section>

          <Section icon="🛡️" title="안전재고 확인">
            <Step n={1} text="사이드바 [본사] → 재고관리 → 안전재고 클릭"/>
            <Step n={2} text="최근 1달간 판매수량 = 안전재고 기준으로 자동 계산됩니다."/>
            <Step n={3} text="현재재고 < 안전재고인 항목은 빨간 배경으로 표시됩니다."/>
            <Step n={4} text="'발주요청' 버튼 클릭 시 해당 항목이 발주요청 목록에 저장됩니다."/>
            <Step n={5} text="'부족 항목만 보기' 버튼으로 재고 부족 항목만 필터링할 수 있습니다."/>
            <Tip text="안전재고 기준은 판매 입력 데이터 기반이므로, 매니저들이 판매를 성실히 입력할수록 정확도가 높아집니다."/>
          </Section>

          <Section icon="👥" title="회원(고객)관리">
            <Step n={1} text="사이드바 [본사] → 회원(고객)관리 클릭"/>
            <Step n={2} text="조건 없이 [조회] 버튼 클릭 시 전체 회원 목록을 테이블로 조회합니다."/>
            <Step n={3} text="이름·연락처 검색, 점포·지점·가입일 필터 조합으로 특정 회원을 빠르게 찾습니다."/>
            <Step n={4} text="회원 행 클릭 시 하단에 해당 회원의 구매이력이 펼쳐집니다."/>
            <Step n={5} text="SMS 동의 회원에게는 '📱 문자' 버튼이 활성화됩니다. (SMS API 연동 후 발송 가능)"/>
            <Tip text="SMS 미동의 회원의 문자 버튼은 클릭해도 발송되지 않습니다."/>
          </Section>

          <Section icon="💰" title="인센티브 조회">
            <Step n={1} text="사이드바 [본사] → 매니저관리 → 인센티브 조회 클릭"/>
            <Step n={2} text="날짜를 선택하고 [조회] 버튼을 누릅니다. 날짜 미선택 시 전체 기간 조회."/>
            <Step n={3} text="매니저별 가입 회원수·총 구매횟수·총 결제금액이 집계됩니다."/>
            <Step n={4} text="'▼ 상세' 버튼으로 해당 매니저의 회원별 구매현황과 상세 내역을 펼쳐볼 수 있습니다."/>
            <Tip text="인센티브는 회원 가입 시 등록된 '담당 매니저 이름' 기준으로 집계됩니다. 동명이인 주의."/>
          </Section>

          <Section icon="📋" title="매출조회">
            <Step n={1} text="사이드바 [본사] → 매출조회 클릭"/>
            <Step n={2} text="점포·브랜드·날짜 필터로 원하는 기간의 판매내역을 조회합니다."/>
            <Step n={3} text="상단 요약에서 총 건수·수량·금액을 바로 확인할 수 있습니다."/>
          </Section>

          <Section icon="🛒" title="판매 입력 (매장)">
            <Step n={1} text="사이드바 [매장] → 판매 입력 클릭"/>
            <Step n={2} text="판매날짜·브랜드·상품·수량·판매가·결제수단을 선택합니다."/>
            <Step n={3} text="회원 연결: '없음' / '기존 회원 검색' / '신규 회원등록' 중 선택"/>
            <Tip text="담당자도 직접 판매 입력이 가능합니다."/>
          </Section>
        </div>
      )}
      ...
      {tab === 'manager' && (
        <div className="card" style={{ padding:'24px 28px' }}>
          <div style={{ marginBottom:20 }}>
            <Badge label="매니저" color="#1565C0"/>
            <span style={{ fontSize:13, color:'var(--text2)', marginLeft:8 }}>매니저는 매장에서 판매입력·회원등록·QR코드 관리를 담당합니다.</span>
          </div>

          <Section icon="🛒" title="판매 입력">
            <Step n={1} text="사이드바 [매장] → 판매 입력 클릭"/>
            <Step n={2} text="판매날짜를 선택합니다. (기본값: 오늘)"/>
            <Step n={3} text="브랜드 선택 → 상품 선택 → 수량 → 판매가 → 결제수단(카드/현금/기타) 순으로 입력"/>
            <Step n={4} text="회원 연결 섹션에서 3가지 중 선택:"/>
            <div style={{ marginLeft:32, marginBottom:8 }}>
              <div style={{ marginBottom:4 }}>• <b>없음</b>: 비회원 판매 (회원 연결 없이 저장)</div>
              <div style={{ marginBottom:4 }}>• <b>기존 회원 검색</b>: 이름·연락처로 검색 후 클릭 선택 → 회원 구매이력에 자동 기록</div>
              <div style={{ marginBottom:4 }}>• <b>신규 회원등록</b>: 이름·연락처·생일·담당 매니저·SMS동의를 입력하면 가입과 동시에 판매 연결</div>
            </div>
            <Step n={5} text="'✓ 판매 입력 저장' 버튼으로 저장. 하단에 최근 10건이 표시됩니다."/>
            <Tip text="판매가는 브랜드/상품 등록 시 설정된 기본가격이 자동 입력됩니다. 수정도 가능합니다."/>
          </Section>

          <Section icon="👤" title="회원 등록">
            <Step n={1} text="사이드바 [매장] → 회원 등록 클릭"/>
            <Step n={2} text="회원가입일·이름·연락처를 입력합니다. (연락처는 010-XXXX-XXXX 형식 자동 변환)"/>
            <Step n={3} text="생일은 선택사항입니다. 입력 시 생일 혜택 문자 발송에 활용됩니다."/>
            <Step n={4} text="담당 매니저 이름을 입력하면 해당 매니저의 인센티브로 집계됩니다."/>
            <Step n={5} text="'광고성 문자 수신 동의' 체크박스: 고객에게 동의 여부를 확인 후 체크"/>
            <Step n={6} text="'✓ 회원 정보 저장' 버튼으로 등록. 하단에 최근 20건이 표시됩니다."/>
            <Tip text="어르신 등 스마트폰 사용이 어려운 고객은 이 화면에서 직접 입력해드리세요."/>
          </Section>

          <Section icon="📋" title="내 회원 목록">
            <Step n={1} text="사이드바 [매장] → 내 회원 목록 클릭"/>
            <Step n={2} text="본인이 담당 매니저로 등록된 회원 목록이 자동으로 표시됩니다."/>
            <Step n={3} text="이름·연락처로 검색하거나, 회원 카드 클릭 시 해당 회원의 구매이력을 확인합니다."/>
            <Step n={4} text="상단에 총 회원수·SMS 동의 회원수·미동의 회원수가 요약됩니다."/>
            <Tip text="내 회원 목록은 '담당 매니저 이름'이 본인 이름과 정확히 일치하는 경우만 표시됩니다."/>
          </Section>

          <Section icon="📱" title="내 QR 코드">
            <Step n={1} text="사이드바 [매장] → 내 QR 코드 클릭"/>
            <Step n={2} text="클릭 즉시 인쇄 화면이 열립니다. 출력 후 매장 카운터에 비치하세요."/>
            <Step n={3} text="고객이 QR코드를 스캔하면 회원가입 페이지로 이동합니다."/>
            <Step n={4} text="고객이 이름·연락처·생일(선택)·SMS동의 여부를 직접 입력하고 등록합니다."/>
            <Step n={5} text="QR로 가입한 고객은 자동으로 해당 매니저와 연결됩니다."/>
            <Tip text="QR코드는 매니저별로 고유합니다. 본인 QR을 다른 매니저와 공유하지 마세요."/>
          </Section>

          <Section icon="📢" title="공지사항">
            <Step n={1} text="사이드바 [공지] → 공지사항 클릭"/>
            <Step n={2} text="관리자가 등록한 공지사항 목록이 표시됩니다."/>
            <Step n={3} text="항목 클릭 시 상세 내용을 확인할 수 있습니다."/>
          </Section>
        </div>
      )}
    </div>
